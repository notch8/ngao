...
```
create secret docker-registry gitlab --docker-server=https://registry.gitlab.com --docker-username={YOUR USERNAME} --docker-password={PERSONAL ACCESS TOKEN} --docker-email={YOUR EMAIL} --namespace {NAMESPACE eg. essi}
```
...
Use `helm fetch`, for example:

```
cd charts
helm fetch stable/mysql --version 1.6.2
```
...
eg. `helm uninstall development --namespace essi`
...
Print your access token in a console as follows, and then copy paste it into the token box on the dashboard login:
```
...
The namespace will be set to the git repository name, eg. essi. Make sure the namespace exists in your cluster. Create it with `kubectl create namespace essi`
